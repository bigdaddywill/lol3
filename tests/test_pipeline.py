...
from pipeline import Bid, Contractor, SearchResult, extract_bid, generate_email, generate_sms, load_contractors, rank_matches, source_field

def make_xlsx(path: Path):
    workbook = '''<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Contractors" sheetId="1" r:id="rId1"/></sheets></workbook>'''
...
def test_workbook_load(tmp_path):
    p=tmp_path/"c.xlsx"; make_xlsx(p); rows,audit=load_contractors(p)
...
def test_match_and_outreach():
    ...
    m=rank_matches(c,[b],minimum_score=1)[0]; subject,email=generate_email(m); sms=generate_sms(m)
...
def test_no_hallucinated_contact():
    ...
    m=rank_matches(c,[b],minimum_score=1)[0]; _,email=generate_email(m)
